...
layer2_input = [
    layer1.output[0:batch_size,0:50,0:2,0:2].flatten(2),
    layer1.output[0:batch_size,0:50,0:2,2:4].flatten(2),
    layer1.output[0:batch_size,0:50,2:4,0:2].flatten(2),
    layer1.output[0:batch_size,0:50,2:4,2:4].flatten(2)
]

# construct a fully-connected sigmoidal layer
layer2 = []
for i in range(4):
    layer2.append(HiddenLayer(
        rng,
        input=layer2_input[i],
        n_in=nkerns[1] * 4,
        n_out=125,
        activation=T.tanh
    ))
layer2_output = T.concatenate([tmp.output for tmp in layer2], axis=1)
...
L2_sqr = (layer0.W ** 2).sum() + (layer1.W ** 2).sum() + (layer3.W ** 2).sum() + sum([(layer2[i].W ** 2).sum() for i in range(4)])
...
#pickle.dump(((layer0.W.get_value(), layer0.b.get_value()), (layer1.W.get_value(), layer1.b.get_value()), (layer2_W.get_value(), layer2_b.get_value()), (layer3.W.get_value(), layer3.b.get_value())), open('model.pkl', 'w'), -1)
